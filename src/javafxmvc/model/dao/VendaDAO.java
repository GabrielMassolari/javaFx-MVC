...
public class VendaDAO {
    ...
    public Map<Integer, Double> listarValorVendasPorMes(int ano) {
        String sql = "select sum(valor), extract(year from data) as ano, extract(month from data) as mes from vendas group by ano, mes having extract(year from data) = ? order by ano, mes;";
        Map<Integer, Double> retorno = new HashMap();
        ...
        try {
            ...
            while (resultado.next()) {
                retorno.put(resultado.getInt("mes"), resultado.getDouble("sum"));
            }
            return retorno;
        } catch (SQLException ex) {
            Logger.getLogger(VendaDAO.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public List<Integer> listarAnosQuePossuemVendas() {
        String sql = "select distinct extract(year from data) as ano from vendas group by ano order by ano;";
